...
approximation and convex outer approximation by domains with
continuously differentiable boundary yield the general theorem.
}
...
\end{equation}
By~\eqref{eq:adjoint-algebra-diagonal}, there is a unique diagonal
analytic function $\Theta$ such that
\[
...
\end{equation}
The denominators are nonzero, and $P$ and $Q$ are Hermitian, because
$\lvert\lambda_i\rvert\leq1$.
...
$\Gamma\subset\Omega$ enclosing $\sigma(B)$.  The compact contours
$\Gamma$ and $\partial\Omega$ are positively separated, so Cauchy's
...
For fixed $w\in\D$, the same formula defines $c_w$ holomorphically on
a neighborhood of $\overline\Omega$: on $\overline\Omega$ one has
$\lvert1-wf\rvert\geq1-\lvert w\rvert>0$.  Hence
Lemma~\ref{lem:double-layer-map} gives
\[
  2H(w)=c_w(B)+g_{c_w}(B)^*.
\]
...
\begin{equation}
  H(w)-(I-wT)^{-1}
  =\frac12\bigl(g_{c_w}(B)^*-I\bigr).
  \label{eq:direct-completion}
\end{equation}
The function $g_{c_w}$ is holomorphic on a neighborhood of
$\sigma(B)$, so finite-dimensional holomorphic functional calculus
gives $g_{c_w}(B)\in\alg(B)$.  Thus the right-hand side of
\eqref{eq:direct-completion} belongs to $\alg(B^*)$, proving
...
\]
is a bounded convex domain with continuously differentiable boundary,
and $\overline\Omega_\delta=K+\delta\overline\D$.
\end{lemma}
...
domain with continuously differentiable boundary.
...
$K_\delta$ of $K$ that contains no pole of $r$.  Its complement is
connected, so polynomial Runge approximation on $K_\delta$, followed
by the Cauchy integral formula for the functional calculus, gives
...
\cite{OkuboAndo1975}.  Uniform matrix-level spectral-set bounds are
studied in~\cite{HartzMcCarthy2026}, and their relation to numerical
radius in~\cite{DavidsonPaulsenWoerdeman2018}; the constant
...
throughout the final Gramian inequality in the proof of
