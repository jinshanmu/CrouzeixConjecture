...
$0<\lvert q\rvert\leq1$.  The $q$-numerical range and its scaled form
are
\[
...
a maximizing singular-vector pair then extracts the norm at the
identity stretch.  More generally, a universal numerical-range
...
For $n\geq2$ and $0<\lvert q\rvert\leq1$, the $q$-numerical range is
\begin{equation}
 W_q(A)=
...
over precisely these stretches force an improved norm estimate at the
identity stretch.  Third, Section~\ref{sec:main-proof} packages the two
...
After division by $r$, Tsing's Lemma 5 gives the following disk formula
\cite{Tsing1984}.  It will also be used in the sharpness argument.  For
a unit vector $x$, put
\[
 \rho_A(x)=
...
\eqref{eq:rank-one-stretch} into a sharper norm bound at the identity
stretch.
...
For every fixed $q\in\C$ with $0<\lvert q\rvert\leq1$, put
