...
For a complex square matrix A, we prove that the norm of f(A) is at
most twice the maximum modulus of f on the numerical range of A.
Consequently, the numerical range is a 2-spectral set, and the factor
2 is optimal.  The proof uses a mass-parameterized positive-real
...
Put $K=\overline{W(A)}$; then $\sigma(A)\subset K$.  Write the rational
function as $r=a/b$, where $b$ is zero-free on $K$.  Spectral mapping
makes $b(A)$ invertible.
...
an adjoint-algebra defect, and a simple-spectrum approximation.  A
...
Now let $A\in C_\varrho$ be arbitrary and define, for $z\in\D$,
\[
...
and projection generators of $C(\mathbb T)*_{\C}\C^2$, and put
\[
...
For $c\in\C$ and $0<r<R$, let $A-cI$ be invertible and satisfy
\[
...
\begin{proof}
The upper bound is Theorem~\ref{thm:main} and its Hilbert-space
corollary.  For $0<\delta<1$, let $X_n$ be the cyclic weighted shift
...
\begin{equation}
  H_\upsilon(K)=
  \frac{K^2+1+
  \sqrt{(K^2+1)^2-4\upsilon(2-\upsilon)K^2}}
  {2\upsilon K};
...
\cite{JuryTsikalas2024}, $N+1$ for a disk with $N$
