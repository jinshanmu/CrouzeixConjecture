...
norm.  For every function $f$ holomorphic on a neighborhood of $W(A)$,
we prove that
\[
  \lVert f(A)\rVert\leq
  2\max_{z\in W(A)}\lvert f(z)\rvert
\]
...
relative to an auxiliary eigenbasis: if the resolvent defect of a
normalized matrix-valued Carath\'eodory function lies in the adjoint
algebra, then its underlying matrix has norm at most the mass
$\mathfrak m\geq2$.  The numerical-range double layer has mass two.
...
\label{thm:main}
Let $n\geq1$, let $A\in\Mn$, and let $f$ be holomorphic on a
neighborhood of $W(A)$.  Then
\begin{equation}
  \lVert f(A)\rVert
...
correction's contribution exactly.
...
\end{equation}
The denominators are nonzero because $\lvert\lambda_i\rvert\leq1$.
Since $G=G^*$, conjugate symmetry in the displayed formulas also gives
$P=P^*$ and $Q=Q^*$.
...
$\lambda_i=0$.
We first verify that this sample cancels the contribution of $\Psi$
to the sampled kernel quadratic form.  Its contribution is
\begin{equation}
  2\RePart\sum_{i=1}^n
...
$\mathfrak m>2$, the nonzero constant part
$\Theta(0)=(1-2/\mathfrak m)I$ produces that additional positive
Gramian term; omitting it would invalidate the parameterized proof.
...
Only the canonical parallel domains used in Section~\ref{sec:passage}
are needed.  We say that a bounded convex domain $\Omega$ has a
...
algebra is closed, so $G(w)\in\alg(B)$.  Since
$(I+wT)(I-wT)^{-1}=2(I-wT)^{-1}-I$, we conclude that
\begin{equation}
  H(w)-(I-wT)^{-1}
  =\frac12\bigl(G(w)^*-I\bigr)\in\alg(B^*).
...
\]
If $m_\varepsilon=0$, then $f$ vanishes on a neighborhood of
$\sigma(A)$, so $f(A)=0$ and there is nothing to prove.  Otherwise,
apply Corollary~\ref{cor:auxiliary-bound} to $B_k$ and
...
fixed contour inside $U$, gives $f(B_k)\to f(A)$.  Hence
\begin{equation}
...
compression argument from~\cite{Crouzeix2007}; compare the survey~%
\cite{BickelEtAl2020}.
...
$\sigma(A)\subset K$ will also be used below.  Let $r=a/b$ be a reduced
rational function with no pole on $K$.  Then $b$ has no zero on $K$,
so spectral mapping and $\sigma(A)\subset K$ make $b(A)$ invertible.

The complement of the compact convex set $K$ is connected.  Polynomial
Runge approximation therefore gives polynomials $p_j$ converging
uniformly to $r$ on $K$.  By~\eqref{eq:operator-polynomial-bound},
\[
  \lVert p_j(A)-p_k(A)\rVert
  \leq2\max_{z\in K}\lvert p_j(z)-p_k(z)\rvert,
\]
so $p_j(A)$ converges in operator norm to some $R$, with
\[
  \lVert R\rVert\leq2\max_{z\in K}\lvert r(z)\rvert.
\]
Since $p_jb\to a$ uniformly on $K$, the same polynomial estimate gives
\[
  p_j(A)b(A)=(p_jb)(A)\longrightarrow a(A).
\]
Thus $Rb(A)=a(A)$ and
$R=a(A)b(A)^{-1}=r(A)$.  This proves the rational estimate and hence
the spectral-set assertion.
\end{proof}
...
The proof separates an algebraic completion engine from its geometric
input.  This makes it possible to say precisely when another scalar
spectral-constant problem is ready for the same method.

\subsection{A reusable scalar protocol}
...
\begin{equation}
  ...
  \qquad
  \RePart h\geq0\Longrightarrow\RePart\mathcal P(h)\succeq0,
  \qquad
...
\]
The diagonal entries of $T$ are contractive character values, while
$c_w$ has nonnegative real part.  Hence $H$ is analytic,
$H(0)=I$, and $\RePart H(w)\succeq0$.  Since
\[
  \theta(c_w)=(I+wT)(I-wT)^{-1}
  =2(I-wT)^{-1}-I,
\]
condition~\eqref{eq:discussion-positive-layer} gives
\[
  H(w)-\frac2{\mathfrak m}(I-wT)^{-1}
  =\frac1{\mathfrak m}\bigl(\mathcal P(c_w)-\theta(c_w)-I\bigr)
  \in\alg(B^*).
\]
Theorem~\ref{thm:completion} yields
$\lVert\theta(f)\rVert\leq\mathfrak m$; take the supremum over the
unit ball of $\mathcal A$.
\end{proof}

Thus a finite-dimensional scalar problem is ready once four items are
available: a positive layer, its total mass, an adjoint-algebra defect,
and an approximation by simultaneous simple-spectrum models.  The
upper bound produced by the method is the layer mass, not the number
of boundary components by itself.
Proposition~\ref{prop:positive-layer-transfer} also resolves
the finite-dimensional semisimple-range case of the abstract
Crouzeix problem in~\cite[Conjecture~1.2]{ClouatreOstermannRansford2023}:
if $\alpha$ is the unital antilinear contraction in that conjecture and
\[
  \theta_\alpha(h)=\frac12\bigl(
  \theta(h)+\theta(\alpha(h))^*\bigr)
\]
is contractive, then $\mathcal P=2\theta_\alpha$ has mass two.  Indeed,
each map $h\mapsto x^*\theta_\alpha(h)x$, for $\lVert x\rVert=1$, is a
unital functional of norm one, hence extends to a state on $C(K)$ and
preserves nonnegative real parts.  Moreover,
$\mathcal P(h)-\theta(h)=\theta(\alpha(h))^*$.  Simultaneous
diagonalization of the semisimple range, followed by the choice of an
auxiliary simple diagonal matrix, verifies the remaining hypothesis.

\subsection{Formal scalar consequences}

Several consequences require no new boundary layer.  A faithful
universal representation of a unital $C^*$-algebra $\mathfrak C$
transfers Corollary~\ref{cor:operator-bound} to its algebraic numerical
range $V_{\mathfrak C}(a)$:
\begin{equation}
  \lVert f(a)\rVert\leq
  2\max_{z\in V_{\mathfrak C}(a)}\lvert f(z)\rvert.
  \label{eq:Cstar-discussion}
\end{equation}
Here $f$ may be any rational function with poles off
$V_{\mathfrak C}(a)$.  Applying this to the Calkin algebra gives
\[
  \lVert f([A])\rVert\leq
  2\max_{z\in W_{\mathrm e}(A)}\lvert f(z)\rvert
\]
for every rational $f$ with poles off $W_{\mathrm e}(A)$, where $[A]$
is the Calkin coset.  If $f$ is also pole-free on $\sigma(A)$, the
left side equals $\lVert f(A)\rVert_{\mathrm e}$.  For separable
infinite-dimensional Hilbert spaces,
\cite[Theorem~5.2]{BlazhkoEtAl2025} identifies the universal Calkin
constant with the universal Hilbert-space constant, and hence yields
the same conclusion once Corollary~\ref{cor:operator-bound} is used.

For a closed densely defined operator satisfying
$\sigma(A)\subset W(A)$ and
$\overline{W(A)}\subset\Omega$, where
$\Omega\neq\C$ is an open unbounded convex domain of aperture
$2\alpha$, $0\leq\alpha\leq\pi/2$, the Yosida approximation in~%
\cite[Section~3]{CrouzeixUnbounded2025} combines with
Corollary~\ref{cor:operator-bound} to give, for every rational $f$
bounded on
$\Omega$ and with poles off $\sigma(A)$,
\begin{equation}
 \lVert f(A)\rVert\leq
 \min\left\{2,
 1-\frac{\alpha}{\pi}
 +\sqrt{2-\frac{4\alpha}{\pi}+\frac{\alpha^2}{\pi^2}}
 \right\}
 \sup_{z\in\Omega}\lvert f(z)\rvert.
 \label{eq:unbounded-discussion}
\end{equation}
The value $2$ improves the aperture estimate exactly when
$0\leq\alpha<\pi/6$.

Operator-radius constants transfer formally as well.  The conversion
theorem of Badea, Crouzeix, and Klaja gives, from a sharp scalar norm
constant two,
\begin{equation}
  H_\rho(2)=
  \frac{5+\sqrt{25-16\rho(2-\rho)}}{4\rho},
  \qquad \rho\geq1;
  \qquad H_2(2)=\frac54.
  \label{eq:operator-radius-discussion}
\end{equation}
See~\cite[Theorems~3.2 and~5.1]{BadeaCrouzeixKlaja2018}.

\subsection{New mass-two layers and short geometric transfers}

The quantum annulus is the cleanest genuinely new positive-layer
test.  Fix $R>1$ and orient the two circles of
$\mathbb A_R=\{R^{-1}<\lvert z\rvert<R\}$ positively.  If $B$ is
invertible with $\lVert B\rVert,\lVert B^{-1}\rVert<R$, subtract from
the annular double-layer density
\[
  \mu_B(z)=\frac1{2\pi\iu}\left[
  z'(s)(zI-B)^{-1}
  -\overline{z'(s)}(\overline zI-B^*)^{-1}\right]
\]
the scalar winding density
\begin{equation}
  \mathcal D_B^{\rm ann}(z)
  =\mu_B(z)-\frac1{2\pi\iu}\frac{z'(s)}zI.
  \label{eq:annular-shifted-layer}
\end{equation}
On the outer circle and on the clockwise inner circle, with
$r=R^{-1}$, direct multiplication gives respectively
\begin{align}
 2\pi R\mathcal D_B^{\rm ann}(z)
 &= (zI-B)^{-1}(R^2I-BB^*)
...
\end{align}
The scalar winding numbers cancel, so this layer has mass $2I$; the
annular Cauchy identity leaves an adjoint Cauchy companion in
$\alg(B^*)$.  If $B$ has simple spectrum, all hypotheses of
Proposition~\ref{prop:positive-layer-transfer} are therefore present.
Arbitrary matrices in the strict class follow by simple-spectrum
approximation, and radius enlargement gives the non-strict boundary
case.  This yields the finite-dimensional upper constant two.
Tsikalas's bilateral weighted
shift gives the matching lower bound for the universal
all-Hilbert-space constant~\cite{Tsikalas2022}; it does not by itself
prove finite-dimensional sharpness.  For context, the previous
all-Hilbert-space upper bound was $1+\sqrt2$
\cite{CrouzeixGreenbaum2019}, and a more recent matrix bound is
$1+\sqrt{1+2/(R^2+1)}$~\cite{CrouzeixAnnuli2025}.  The boundary
dilation of McCullough and Pascoe, together with residual finite
dimensionality of the relevant free product, can be combined in a
companion argument to pass from matrices to arbitrary Hilbert spaces
\cite[Theorem~1.1 and Proposition~3.2]{McCulloughPascoe2023},
\cite[Theorems~2.4 and~3.2]{ExelLoring1992}.

The centered annular classes of Jury and Tsikalas provide a second
ready input.  Their contractive double-layer calculus has mass two
when $c=0$ and mass $2+c$ in general; compare their
Definition~3.6 and Theorem~3.12 in~\cite{JuryTsikalas2024}.
Theorem~3.12 assumes that the spectrum lies in the open annulus; the
closed-boundary class requires radius enlargement followed by a stable
limit.  Subject to that reduction, at $c=0$ the mass-two theorem
supplies the finite-dimensional upper constant two, improving their
general $1+\sqrt2$ estimate
\cite[Theorem~5.3]{JuryTsikalas2024} and extending their special
$2\times2$ constant-two result
\cite[Theorem~1.3]{JuryTsikalas2024}.  For $c>0$, in finite dimensions,
Theorem~\ref{thm:completion} supplies the upper coefficient $2+c$,
without a sharpness assertion.

Other problems invite a short transfer from the numerical-range
theorem rather than a new layer.  For $A\in\Mn$ and
$0<\lvert q\rvert\leq1$, put
\[
  W_q(A)=\{\langle Ax,y\rangle:
  \lVert x\rVert=\lVert y\rVert=1,\ \langle x,y\rangle=q\},
  \qquad \Omega_q=q^{-1}W_q(A).
\]
Tsing proves the convexity of $W_q(A)$~\cite{Tsing1984}, while
O'Loughlin and Rani propose in
\cite[Conjecture~4.2]{OLoughlinRani2026} the estimate
$\lVert p(A)\rVert\leq C_q
\sup_{z\in\Omega_q}\lvert p(z)\rvert$, with the candidate
\begin{equation}
  C_q=\max\left\{1,
  \frac{2\lvert q\rvert}{1+\sqrt{1-\lvert q\rvert^2}}\right\}.
  \label{eq:q-candidate-discussion}
\end{equation}
Deriving this value from Theorem~\ref{thm:main} requires a separate
rank-one-similarity and scalar-extraction argument; that step is not
contained in the two cited sources.
Likewise, every ellipse containing $W(A)$ has upper constant two
immediately;
fixed-ellipse sharpness and the Douglas--Paulsen norm require the
structured ellipse and annular models in
\cite{AglerLykovaYoung2024,AglerLykovaYoung2025}.
The two-disk setup in~\cite[Theorem~10]{CrouzeixGreenbaum2019}
assumes $\lVert A-w_1I\rVert\leq R_1$ and
$\lVert(A-w_2I)^{-1}\rVert\leq R_2$, and distinguishes nested,
crossing, and tangent boundary circles.
For this setup the crossing case admits a M\"obius reduction to a
convex disk--half-plane geometry, while a genuine nested annular case
reduces to the quantum annulus.

\subsection{Higher-mass scalar problems}

The mass theorem also identifies several finite-dimensional problems
whose algebraic part is complete and whose remaining work is boundary
factorization and approximation.
\begin{enumerate}
  \item For a disk with $N$ disjoint circular holes, subtracting one
  unit of mass on the outer circle and adding one on each inner circle
  leaves a positive layer of mass $N+1$.  The resulting target bound is
  $N+1$.  The underlying class originates in
  \cite{BadeaBeckermannCrouzeix2009}.  A recent geometry-independent
  bound is
  $(N+1)/2+\sqrt{(N+1)^2/4+N}$, and the question whether $N+1$ is a
  universal lower bound appears in
  \cite[Theorem~2.3 and Question~2.5]{TsikalasMultiplyConnected2026}.
  Sharpness remains open for $N\geq2$.
  \item For the numerical annulus
  $w(A)\leq R$, $w(A^{-1})\leq R$, the outer convex layer has mass two
  and the shifted inner layer adds two, producing the target upper
  bound four.  The universal all-Hilbert-space lower bound is two by
  inclusion of the quantum-annulus class~\cite{Tsikalas2022}, while
  the recent matrix estimate is
  $2+\sqrt{4+2/(R^2+1)}$~\cite{CrouzeixAnnuli2025}; the sharp
  finite-dimensional constant remains open.
  \item For $\mathrm{DL}\mathbb A_R(c)$ with $c>0$, the natural mass is
  $2+c$, as above.  Its optimality is not implied by the completion
  theorem.
\end{enumerate}
Polyannuli and intersections of three or more arbitrary spherical
disks are not automatic consequences: they introduce several kernels
or fail to reduce to one annulus or one convex numerical-range
problem.  Nor do the finite-dimensional higher-mass arguments pass to
arbitrary Hilbert spaces without a separate stable reduction.

\subsection{The matrix-valued obstruction and extremizers}

The principal strengthening still outside the method is the uniform
matrix-polynomial question: for every $m\geq1$ and every
...
\end{equation}
The case $m=1$ is Theorem~\ref{thm:main}; uniformity in $m$ is an
additional operator-algebraic requirement.  At matrix level the
correction occupies $\alg(B^*)\otimes\mathbb M_m$, so its diagonal
entries are operator blocks and the scalar cancellation in
\eqref{eq:theta-contribution} no longer yields the Gramian inequality.
The foundational disk similarity estimate is due to Okubo and Ando~%
\cite{OkuboAndo1975}; see also
\cite{DavidsonPaulsenWoerdeman2018,HartzMcCarthy2026}.  The complete
$1+\sqrt2$ bound follows from~\cite{CrouzeixPalencia2017}, and complete
constant two is known for the weighted-shift family
\cite{CrouzeixGreenbaum2025}.  Thus no complete constant-two,
similarity, or normal-dilation conclusion is asserted here.

Finally, specialize the proof of Theorem~\ref{thm:completion} to mass
two.  If $\lVert\widetilde T\rVert=2$ and a unit vector $e$ satisfies
$\lVert\widetilde Te\rVert_2=2$, then
\eqref{eq:Phat-bounds} and~\eqref{eq:two-gramians} imply
\[
  \widehat Pe=2e,
  \qquad \widetilde T^2e=0,
  \qquad \widehat P\widetilde Te=\widetilde Te.
\]
These saturation conditions may classify extremal pairs and quantify
stability near~\eqref{eq:sharp-example}; any classification must allow
the nonuniqueness phenomena in~\cite{Li2020}.
