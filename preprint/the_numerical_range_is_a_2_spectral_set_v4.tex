...
term's contribution and leaves a comparison of two weighted Gramians;
their first nonconstant terms yield the sharp estimate.  Simple-spectrum
approximation and canonical convex outer domains with supported oriented
radial boundaries yield the general theorem.
}
...
followed by the first nonconstant Gramian term yields
$\lVert T\rVert\leq2$.
\end{enumerate}
...
and put $G=S^*S\succ0$.  Since the $\beta_i$ are distinct, polynomial
...
\[
  \Theta(w)=S^*\bigl(H(w)-(I-wT)^{-1}\bigr)(S^*)^{-1}
\]
is diagonal for every $w$, and
\[
  H(w)-(I-wT)^{-1}=(S^{-1})^*\Theta(w)S^*.
\]
Also $\Theta(0)=0$ because $H(0)=I$.
...
$\lambda_i=0$.
We first verify that this sample cancels the contribution of $\Theta$
to the sampled kernel quadratic form.  The contribution of the second
term in~\eqref{eq:Htilde-decomposition} to the kernel quadratic form is
\begin{equation}
  2\RePart\sum_{i=1}^n
...
$G(I-w\Lambda)^{-1}$.  For sample indices $r,s\in\{1,\ldots,n\}$,
compression of the kernel block between $w_r$ and $w_s$ against
$e_r$ and $e_s$ gives
\begin{equation}
  \frac{2G_{rs}}
  {(1-\overline{\lambda_r}\lambda_s/2)
   (1-\overline{\lambda_r}\lambda_s/4)}
  =(4Q-2P)_{rs}.
  \label{eq:sampled-main-block}
\end{equation}
After compression by the coordinate vectors $e_i$, the two effective
sample--origin blocks are $G+Q$, and the origin block is $2G$.  After
the cancellation of $\Theta$'s contribution, kernel positivity gives,
for the prescribed vector $v=-G^{-1}Pu$,
\begin{equation}
\begin{aligned}
0&\leq
...
\end{equation}
Here the block quadratic form is used only on the graph
$v=-G^{-1}Pu$; it is not asserted to be positive for independent
$u$ and $v$.  The last equality uses $Q=P+Y$.
...
\eqref{eq:eigenvector-evaluation} forces $e^*\widehat Ye=0$.  The
$k=1$ term in~\eqref{eq:Yhat-positive} is
$\widetilde T^*\widetilde T/4$, so $\widetilde Te=0$.  The first
series in~\eqref{eq:two-gramians} gives $\widehat Pe=e$, contradicting
...
bounds $\widehat P$, and the first nonconstant term of $\widehat P$
then gives the sharp constant.  The origin sample cancels the
correction's contribution by an exact algebraic identity.  Distinct
auxiliary eigenvalues identify
...
\end{equation}
For $\varphi\in C([0,2\pi])$, define
\begin{equation}
  \Phi_\gamma(\varphi)=\frac12\int_0^{2\pi}
  \varphi(t)D_B(t)\,dt.
  \label{eq:Phi-definition}
\end{equation}
The positivity of $D_B$ and~\eqref{eq:double-layer-mass} make $\Phi_\gamma$
positive and unital, hence $*$-preserving.  It is bounded because
\[
...
foundational disk similarity estimate is due to Okubo and Ando~%
\cite{OkuboAndo1975}; its modern complete-spectral-set formulation and
relation to numerical radius are discussed in~%
\cite{DavidsonPaulsenWoerdeman2018}.  Uniform matrix-level spectral-set
bounds are studied in~\cite{HartzMcCarthy2026}; the constant
...
$\lVert e\rVert_2=1$ and $\lVert\widetilde Te\rVert_2=2$, then
\eqref{eq:Phat-bounds} and the first series in~\eqref{eq:two-gramians}
imply
\[
  \widehat Pe=2e,
...
scaled $q$-numerical ranges, $0<\lvert q\rvert\leq1$, and propose associated
spectral-set problems~\cite{OLoughlinRani2026}.  These provide a natural
testing ground for the conjectured sharp $q$-dependent refinement of the
constant-$2$ bound.
...
that cancels the adjoint-algebra correction's contribution; this
appears in the proof of Theorem~\ref{thm:completion}, especially~%
